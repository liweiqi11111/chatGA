...
async def list_kbs(current_user: User = Depends(get_current_user)):
    # Get List of Knowledge Base from user_id
    user_id = current_user.user_id
    return ListResponse(data=list_kbs_from_db(user_id))
...
async def create_kb(knowledge_base_name: str = Body(..., examples=["samples"]),
    ...
    kb = KBServiceFactory.get_service(user_id, knowledge_base_name, vector_store_type, embed_model)
    kb.create_kb()
